...
function BuyerCars() {
  ...
  const filteredCars = cars.filter(car =>
    car.VehicleModel.toLowerCase().includes(searchTerm.toLowerCase()) // Fixed: Use VehicleModel instead of name
  );
              ...
              <Card sx={{ height: '100%', display: 'flex', flexDirection: 'column', boxShadow: 6, borderRadius: 3, transition: '0.3s', '&:hover': { transform: 'scale(1.05)' } }}>
                ...
                <CardContent sx={{ flexGrow: 1, textAlign: 'center' }}>
                  <Typography 
                    ...
                    color="primary"
                  >
                    {car.VehicleModel} {/* Fixed: Use VehicleModel instead of name */}
                  </Typography>
                  <Typography 
                    variant="h6" 
